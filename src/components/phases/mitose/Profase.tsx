...
export default function Profase() {
  return (
    <group>
      ...
      <Chromosome position={[-0.4, 0.2, 0]} rotation={[0, 0, Math.PI / 6]} color="#ef4444" />
      <Chromosome position={[0.3, 0.3, 0.2]} rotation={[0, 0, -Math.PI / 4]} color="#ef4444" />
      <Chromosome position={[-0.2, -0.3, -0.1]} rotation={[0, 0, Math.PI / 3]} color="#ef4444" />
      <Chromosome position={[0.4, -0.2, 0.1]} rotation={[0, 0, -Math.PI / 5]} color="#ef4444" />
      
      {/* Spindle fibers forming from poles */}
      <SpindleFiber from={[-2.2, 0, 0]} to={[-0.4, 0.2, 0]} opacity={0.4} />
      <SpindleFiber from={[-2.2, 0, 0]} to={[0.3, 0.3, 0.2]} opacity={0.3} />
      <SpindleFiber from={[2.2, 0, 0]} to={[-0.2, -0.3, -0.1]} opacity={0.4} />
      <SpindleFiber from={[2.2, 0, 0]} to={[0.4, -0.2, 0.1]} opacity={0.3} />
      
      {/* Portuguese labels */}
      <Label3D position={[0, 2.2, 0]} text="Cromossomos Condensados" visible={true} />
      <Label3D position={[-2.5, 0.8, 0]} text="Fuso Mitótico Formando" visible={true} />
      <Label3D position={[0, -2.5, 0]} text="Membrana Nuclear Desaparecendo" visible={true} />
    </group>
  )
}
